...
export class RaiPipe implements PipeTransform {
  ...
  transform(value: any, args?: any): any {
    ...
    switch (denomination.toLowerCase()) {
      default:
      case 'rai': return `${(value / this.mrai).toFixed(6)}${!hideText ? ' RAICOIN' : ''}`;
      case 'mrai':
        ...
        if (hasRawValue) {
          ...
          return `${this.toFixed(newVal, this.precision)}${!hideText ? ' RAICOIN' : ''}`;
        } else {
          return `${(value / this.mrai).toFixed(6)}${!hideText ? ' RAICOIN' : ''}`;
        }
